...
resource "docker_image" "document_handler_build" {
  ...
  name = "${var.region}-docker.pkg.dev/${var.project}/${var.cloud_run_repository_name}/${var.document_handler_docker_image}:version_1"
  build {
...
resource "docker_image" "image_handler_build" {
  provider = docker.docker_images
  name = "${var.region}-docker.pkg.dev/${var.project}/${var.cloud_run_repository_name}/${var.image_handler_docker_image}:version_1"
  build {
...
resource "docker_image" "video_handler_build" {
  provider = docker.docker_images
  name = "${var.region}-docker.pkg.dev/${var.project}/${var.cloud_run_repository_name}/${var.video_handler_docker_image}:version_1"
  build {
